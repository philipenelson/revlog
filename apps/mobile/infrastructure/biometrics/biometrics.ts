...
// The OS biometric prompt is a native modal that Appium cannot reliably drive.
// Behind the E2E build flag we stub it so the unlock flow (enable → restart →
// auto-unlock → Garage) runs deterministically on both platforms. Read per
// call so Jest can toggle it; Metro inlines EXPO_PUBLIC_* at build time, so in
// the app bundle this collapses to a constant. See ADR 0036.
