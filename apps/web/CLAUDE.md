...
Routes follow Next.js App Router conventions under `src/app/`. Route groups use parentheses — `(auth)` groups login/register screens without adding a URL segment.
...
src/app/
  ...
  globals.css       ← CSS custom properties from tokens + Tailwind bridge
```
...
Use the shared client logger at `apps/web/src/lib/logger.ts`:
...
- Import `logger` from `@/lib/logger`, not from `console`
