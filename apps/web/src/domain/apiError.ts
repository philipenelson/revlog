...
// Shared pure helpers for classifying API failures (ADR 0043). No React, no I/O.
// Viewmodel hooks call these to decide what to show and whether to log.
...
export const OTP_CODE_EXPIRED = "That code has expired or been used up. Request a new one below.";
...
// The OTP submit-error mapping shared by the verify-email and reset-password
// flows: a known code slug maps to friendly copy (not logged); anything else is
// the generic service error, logged only when it isn't a plain 4xx.
