...
| No "remove photo to none" affordance | The `×` on a freshly-picked preview only discards *that pending pick*, reverting to the currently-saved photo (or the empty placeholder if there was none) — it never calls the API | There is no API endpoint to clear an existing photo back to null. First cut while implementing without checking in — confirmed with the Owner afterward as the correct scope, not just an assumption: the ask was the ability to *change* the photo, not remove it, and this stays deferred rather than added now |
